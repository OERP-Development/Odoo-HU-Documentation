...
If a printer does not show up in the devices list, go to the :abbr:`IoT (Internet of Things)` box
...
the printer in the list. In the :guilabel:`Administration` drop-down, click on :guilabel:`Modify
...
.. spoiler::
   ...
   However, when trying to print with the naming convention: `EpsonTMU220B__IMC_LDV__`, it prints the
   receipt, but it is too big and outside the margin. To resolve this, add a new printer (and naming
...
Barcode scanner processes every barcode character individually
--------------------------------------------------------------

When accessing the mobile version of Odoo from a mobile device or tablet paired with a barcode
scanner via the :abbr:`IoT (Internet of Things)` box, the scanner may process each barcode character
as an individual scan. In this case the :guilabel:`Keyboard Layout` option needs to be filled out
with the appropriate language of the barcode scanner on the :guilabel:`Barcode Scanner` form page.

.. tip::
   Access the barcode scanner by navigating to :menuselection:`IoT App --> Devices --> Barcode
   Scanner`.
...
The keyboard layout is language based, and the options available will vary depending on the device
and the language of the database. For example: `English (UK)`, `English (US)`, etc.
